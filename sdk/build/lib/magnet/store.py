...
_PROFILE_INDEX_PREFIX = "vmm:profile_index:"
...
class ProfileStore:
    ...
    def register_profile(
        ...
        """Track that a profile exists for this user/project pair (Redis only)."""
        if not self._redis:
            return
        try:
            index_key = _PROFILE_INDEX_PREFIX + user_id
            field = f"{profile_id}:{project_id}"
            value = json.dumps(
                {"pref_count": pref_count, "updated_at": time.time()},
                ensure_ascii=False,
            )
            self._redis.hset(index_key, field, value)
        except Exception as e:
            logger.debug(f"register_profile: {e}")

    def list_user_profiles(self, user_id: str) -> list[dict]:
        """Return a summary list of all profiles recorded for a user."""
        if not self._redis:
            return []
        try:
            index_key = _PROFILE_INDEX_PREFIX + user_id
            all_fields = self._redis.hgetall(index_key)
            if not all_fields:
                return []
            ...
            for field, raw in all_fields.items():
                try:
                    profile_id, project_id = field.split(":", 1)
                    ...
                    profiles[profile_id]["projects"].append(project_id)
                    profiles[profile_id]["preference_count"] += meta.get("pref_count", 0)
                except Exception:
                    continue

            return sorted(profiles.values(), key=lambda p: p["profile_id"])
        except Exception as e:
            logger.debug(f"list_user_profiles: {e}")
            return []
